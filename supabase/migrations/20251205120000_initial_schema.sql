-- Base schema for Multicell Supabase project

set check_function_bodies = off;

create extension if not exists "pgcrypto";

-- Helper functions to resolve the current owner based on JWT
...
-- Triggers
create trigger proprietarios_set_dual_ts
before insert or update on public.proprietarios
for each row execute procedure public.set_dual_timestamps();

create trigger clientes_set_dual_ts
before insert or update on public.clientes
for each row execute procedure public.set_dual_timestamps();

create trigger produtos_set_dual_ts
before insert or update on public.produtos
for each row execute procedure public.set_dual_timestamps();

create trigger vendas_set_dual_ts
before insert or update on public.vendas
for each row execute procedure public.set_dual_timestamps();

create trigger itens_venda_set_dual_ts
before insert or update on public.itens_venda
for each row execute procedure public.set_dual_timestamps();

create trigger caixa_set_dual_ts
before insert or update on public.caixa_movimentos
for each row execute procedure public.set_dual_timestamps();

create trigger os_set_dual_ts
before insert or update on public.os
for each row execute procedure public.set_dual_timestamps();

create trigger os_sync_alias
before insert or update on public.os
for each row execute procedure public.os_sync_aliases();

create trigger garantias_set_dual_ts
before insert or update on public.garantias
for each row execute procedure public.set_dual_timestamps();

create trigger configuracoes_set_dual_ts
before insert or update on public.configuracoes
for each row execute procedure public.set_dual_timestamps();

create trigger storage_files_created_ts
before insert on public.storage_files
for each row execute procedure public.set_created_only();

create trigger despesas_set_dual_ts
before insert or update on public.despesas
for each row execute procedure public.set_dual_timestamps();

create trigger usuarios_set_dual_ts
